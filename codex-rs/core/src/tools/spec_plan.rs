...
fn collect_tool_executors(
        ...
        && let Some(apply_patch_tool_type) = &config.apply_patch_tool_type
    {
        let include_environment_id =
            matches!(config.environment_mode, ToolEnvironmentMode::Multiple);
        executors.push(Arc::new(ApplyPatchHandler::new(
    ...
    {
        executors.push(Arc::new(GrepFilesHandler));
    }
    if config
        .experimental_supported_tools
        .iter()
    ...
    {
        executors.push(Arc::new(ReadFileHandler));
    }
    if config
        .experimental_supported_tools
        .iter()
    ...
    {
        executors.push(Arc::new(ListDirHandler));
    }
