    ...
    pub(crate) async fn dispatch_any(
        ...
        match result {
            Ok(_) => {
                ...
                let maybe_tool_call_item =
                    should_emit_tool_call_item(tool_name.name.as_str(), &result.payload).then(
                        || {
                ...
                if let Some(Some(item)) = maybe_tool_call_item {
                    invocation
                        .session
...
fn build_function_tool_call_item(
    ...
    use codex_protocol::items::FunctionToolCallItem;
